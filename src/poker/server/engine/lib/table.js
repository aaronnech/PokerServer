...
Table.prototype.dealCards = function(total) {
    ...
    for (var i = 0; i < total; i++) {
        var card = this.game.deck.pop();
        this.game.board.push(card); //Turn a card
        result.push(card);
    }
...
Table.prototype.setStep = function(step) {
    ...
    switch(step) {
        ...
        case 'flop':
            ...
            var cards = this.dealCards(3);
            for (var i = 0; i < cards.length; i++) {
                this.emit("showCard", cards[i]);
            }
